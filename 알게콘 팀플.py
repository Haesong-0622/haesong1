def main():
    while True:
        ...
        # 선택에 따라 반응 출력 (선택한 번호가 유효한지 확인)
        if choice == '1':
            print("MBTI 테스트를 선택하셨습니다.")
            mbti_test()
        elif choice == '2':
            print("사이코패스 테스트를 선택하셨습니다.")
            psychopath_test()
        elif choice == '3':
            print("가치관 심리 테스트를 선택하셨습니다.")
            value_test()
        elif choice == '4':
            print("성인애착유형 테스트를 선택하셨습니다.")
            attachment_test()
        elif choice == '5':
            print("연애 심리 테스트를 선택하셨습니다.")
            love_psychology_test()

        else:
            print("잘못된 선택입니다. 1에서 5 사이의 번호를 선택해 주세요.")
            continue
...
def continue_or_exit():
    while True:
        ...
        if choice == '1':
            ...
            print("잘못된 선택입니다. 1 또는 2를 선택해 주세요.")

def mbti_test():
    questions = [
        ("친구들과 놀이공원에 간 나, 롤러코스터를 타기 전 내 머릿속은?\n a. 두근두근!! 재밌겠다 & 무섭다.\n b. 이거 안전바 풀리는 거 아냐?", 's', 'n'),
        ("롤러코스터가 출발하고 알바생들이 손인사를 한다. 내 반응은?\n a. 격하게 손을 흔들며 신나한다.\n b. 어색하게 흔든다 & 안 흔든다.", 'e', 'i'),
        ("놀이 기구를 타고 기념품 숍에 갔는데 친구가 쓸데없는 선물을 서로 사주자고 한다.\n a. 좋아 재밌겠다 추억이 될 듯\n b. 비싼데 뭐 하러 쓸데없는걸?", 'f', 't'),
        ("배가 고파서 밥을 먹으러 가는 우리, 내가 알아본 식당은?\n a. 맛있다는 곳 1군데\n b. 변수가 생길지도 모르니 2군데 이상", 'p', 'j'),
        ("밥을 먹다가 무서운 얘기가 나왔다. 여기 사실 숨겨진 놀이 기구가 있대.. 겪어본 사람이 있대\n a. 에이~(내가 겪지 않았으니 안 믿음)\n b. 헐!!(흥미진진 벌써 빠져들었음)", 's', 'n'),
        ("놀이공원 퍼레이드를 보러 온 우리, 친구들이 춤을 추기 시작한다.\n a. 당연히 나도 따라 춤을 춘다.\n b. 실시간으로 기가 빨린다.", 'e', 'i'),
        ("이제 집에 갈 시간, 친구의 이 말에 기분이 좋아졌다.\n a. 너랑 놀아서 너무 행복했어\n b. 너랑 놀면 똑똑해서 손해 볼 일이 없다.", 'f', 't'),
        ("집에 가는 길, 친구가 방향이 같다고 같이 가자고 한다.\n a. 그래 좋지! 가는 길 안 심심하겠다.\n b. 그래~(집은 혼자 가고 싶었는데)", 'e', 'i'),
        ("친구와 대화 중에 초능력 드라마 얘기가 나왔다.\n a. 드라마가 재밌는지에 초점을 두고 얘기한다.\n b. 초능력에 대해 초점을 두고 얘기한다.", 's', 'n'),
        ("요즘 사는 게 어떻냐는 친구의 질문에 나는\n a. 내 인생에 특별한 이벤트가 생겼으면 좋겠어\n b. 난 그냥 매일 같은 하루였으면 좋겠어", 'p', 'j'),
        ("친구와 헤어지려는데 갑자기 난 그냥 네가 너무 좋아라고 한다.\n a. 친구의 말에 감동을 받는다.\n b. (그냥? 왜? 갑자기? 이유가 뭐지)", 'f', 't'),
        ("집에 돌아와 내일 알람 시간을 맞추는 나, 지금 내 머릿속엔?\n a. 아 지금 자면 몇 시간 잘 수 있지?\n b. 몇 시에 일어나서 몇 시까지 준비하면 되겠다.", 'p', 'j')
    ]
    ...
    print(f"\n당신의 MBTI 유형은: {mbti_result}")

def value_test():
    print("\n사랑하는 사이인 음악가(여)와 시인(남)은 강을 사이에 두고 만나지 못합니다.")
    print("강을 건너려면 뱃사공에게 배값을 주어야 하지만 음악가와 시인은 돈이 없어 만날 수가 없습니다.")
    print("음악가는 뱃사공에게 태워달라고 애원했지만, 뱃사공은 배값이 없다면 태워줄 수 없다고 단호하게 거절합니다.")
    print("이때 음악가를 짝사랑하던 사업가가 음악가에게 이런 제안을 합니다.")
    print('"나와 하룻밤을 보내면 배를 탈 수 있는 돈을 주겠소"라고 말입니다.')
    print("시인이 보고 싶은 음악가는 어쩔 수 없이 사업가의 제안을 수락하고 맙니다.")
    print("이를 본 시인의 친구는 음악가와 사업가의 관계를 시인에게 알렸습니다.")
    ...
    print("\n이 다섯 사람 중에 가장 나쁜 사람은 누구라고 생각하시나요? 한 사람을 선택해 주세요.")
    print("1. 사업가와 하룻밤을 보낸 음악가")
    ...
    if choice == '1':
        ...
        result = "잘못된 선택입니다. 1에서 5 사이의 번호를 선택해 주세요."
...
def love_psychology_test():
    ...
    if choice == '1':
        ...
        result = "잘못된 선택입니다. 1에서 4 사이의 번호를 선택해 주세요."
